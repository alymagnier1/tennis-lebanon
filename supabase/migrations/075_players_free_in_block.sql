-- "5 free" had nowhere to go. The liquidity signal (074) reported how many
-- players were free in a block, but nothing could answer the obvious next
-- question -- who? -- so the number was a fact the player could not act on.
--
-- Discover cannot answer it either: its availability filter matches players who
-- overlap the viewer's own availability anywhere in a 14-day horizon, which is a
-- different question and returns a different set. Sending a tap on "5 free" there
-- would have shown 8 people, or 2, and the count on Home would have stopped
-- meaning anything.
--
-- So `discover_compatible_players` gains an optional block window. Extending the
-- existing function rather than writing a sibling is deliberate: the card select
-- underneath is ~100 lines of zones, availability tags, favourite clubs and
-- overlap slots, and a second copy would drift. Recreated from the live
-- definition rather than retyped, with two additions -- the parameters, and one
-- `exists` in the candidate filter.
--
-- The block rule is character-for-character the one `get_availability_liquidity`
-- counts with: at least one contiguous hour inside the block, measured from
-- `greatest(p_free_from, now())` so a block in progress is trimmed to the part
-- still ahead. That is what keeps the count and the list in agreement.
--
-- Dropped first because adding parameters changes the signature: `create or
-- replace` would leave the old function in place and make every existing
-- eight-argument call ambiguous.

drop function if exists public.discover_compatible_players(
  uuid[], public.match_format, public.play_intent, boolean, integer, integer, integer, uuid
);

create or replace function public.discover_compatible_players(p_zone_ids uuid[] DEFAULT NULL::uuid[], p_format match_format DEFAULT NULL::match_format, p_intent play_intent DEFAULT NULL::play_intent, p_require_availability_overlap boolean DEFAULT false, p_horizon_days integer DEFAULT 14, p_level_window integer DEFAULT 1, p_limit integer DEFAULT 20, p_cursor_user_id uuid DEFAULT NULL::uuid, p_free_from timestamptz DEFAULT NULL::timestamptz, p_free_to timestamptz DEFAULT NULL::timestamptz)
 RETURNS SETOF discover_compatible_player_card
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
AS $function$
...
begin
  ...
  with candidates as (
    ...
    where p.id <> v_viewer_id
      ...
      -- Free inside one specific block, when asked. Same rule the liquidity
      -- count uses: at least one contiguous hour of availability within the
      -- block, trimmed to the part still ahead. The count and this list must
      -- agree, so "5 free" always opens onto five players.
      and (
        p_free_from is null
        or p_free_to is null
        or exists (
          select 1
          from public.expand_user_availability(
            p.id,
            greatest(p_free_from, now()),
            p_free_to
          ) as fs
          where extract(epoch from (fs.ends_at - fs.starts_at)) >= 3600
        )
      )
  )
  select
    ...
    public.list_player_favorite_clubs_json(c.user_id) as favorite_clubs
...
revoke all on function public.discover_compatible_players(
  uuid[], public.match_format, public.play_intent, boolean, integer, integer,
  integer, uuid, timestamptz, timestamptz
) from public, anon;
