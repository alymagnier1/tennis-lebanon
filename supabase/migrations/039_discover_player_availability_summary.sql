-- Surface each player's recurring availability on discovery cards when there is
-- no concrete overlap interval to show (viewer has no windows, or schedules differ).

alter type public.discover_compatible_player_card
  add attribute availability_weekdays jsonb;

alter type public.discover_compatible_player_card
  add attribute availability_day_parts jsonb;
