-- Cancelling a match told nobody, and the reason went nowhere.
--
-- `cancel_match` (030) compels the host to give a reason once a match is full,
-- writes it to `matches.cancellation_reason`, and stops. No notification kind
-- existed for cancellation, `get_match_hub` never returned the column, and
-- `list_my_matches` filters cancelled matches out entirely. So the host wrote an
-- explanation for an audience that could not reach it, and the other player
-- watched the match disappear from their list with no word — the same shape as
-- the silent decline that `077` fixed, and the same consequence: a player who
-- turns up, or who stops trusting the list.
--
-- Enqueued from a **trigger on `matches`**, not from inside `cancel_match`.
-- Both `cancel_match` and `withdraw_from_booked_match` (034) can set
-- `status = 'cancelled'`, and the 2026-08-21 entry chose triggers over RPC edits
-- for exactly this reason: an enqueue inside one RPC misses every other path
-- that writes the same state.
--
-- The reason itself stays out of the push payload. `NotificationParams` carries
-- only structured values (`name`, `clubName`, `startsAt`, `spotsLeft`) that each
-- surface renders in the recipient's language; a host-authored string cannot be
-- localized, and putting free text on a lock screen sends it through Expo's
-- servers. The push says a match was cancelled, and the reason is one tap away
-- on the hub.
--
-- The route back matters, because `exitMatchHub` runs after a cancel and
-- `list_my_matches` excludes cancelled rows, so the hub is unreachable from the
-- Matches tab. The durable path is the notification row, which persists and
-- deep-links to `/match/{id}`. `get_match_hub` authorises any participant whose
-- row is still `accepted`, and `cancel_match` only writes `attendance` on the
-- canceller's own row — no participant is demoted — so the link resolves for
-- everyone who needed to hear it.
...
begin
  ...
  -- Null when the transition came from anything but a signed-in caller. That is
  -- not a case today — `cancel_match` and `withdraw_from_booked_match` are the
  -- only writers and both are user-invoked — but `notify_match_participants`
  -- treats a null exclusion as "notify everyone", which is the right fallback
  -- if a future sweeper ever cancels on nobody's behalf.
  v_actor_id := auth.uid();

  -- Same 15-minute bucket as the roster events. A match cancels once, so this
  -- collapses nothing in practice; it is kept for shape consistency with the
  -- other enqueues rather than because it does work here.
...
create trigger matches_notify_cancelled
