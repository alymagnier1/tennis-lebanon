-- Match list cards need club + area before a court is booked.
--
-- list_my_matches only returned club_name from an accepted booking, so a
-- ready_to_book card showed format alone even though the host had already
-- named preferred clubs and zones. Preferred clubs and match zones mirror the
-- jsonb shape used by get_match_hub / discover_open_match_card so the mobile
-- helpers (clubLabelFromList / zoneLabelFromList) can render both chips.
