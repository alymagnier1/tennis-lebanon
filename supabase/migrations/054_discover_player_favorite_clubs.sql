-- Expose a player's favourite clubs on discovery and public player cards.

create or replace function public.list_player_favorite_clubs_json(p_user_id uuid)
returns jsonb
language sql
stable
security definer
...
as $$
  ...
  join public.clubs as c on c.id = pfc.club_id
  where pfc.user_id = p_user_id
    and c.is_active = true;
$$;

revoke all on function public.list_player_favorite_clubs_json(uuid) from public, anon;
grant execute on function public.list_player_favorite_clubs_json(uuid) to authenticated;

alter type public.discover_compatible_player_card
  add attribute favorite_clubs jsonb;
...
begin
  ...
  select
    ...
    public.list_player_favorite_clubs_json(p.id)
  into v_card
  from public.profiles as p
  join public.player_profiles as pp on pp.user_id = p.id
