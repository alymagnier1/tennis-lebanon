-- The hub knew every player's face and sent none of them.
--
-- `MatchHubParticipants` passes `participant.avatar_path` to `Avatar`, and
-- `HubParticipant` declares the field — but `get_match_hub` never returned it,
-- so every avatar on the hub fell back to initials. The UI has been wired for
-- photos it was never given.
--
-- It matters most on the pending-request card. A host deciding whether to
-- accept a stranger is deciding about a person, and a name alone is thin
-- grounds; the profile behind it carries level, areas and matches played. The
-- card now leads with a face and opens that profile.
--
-- Added to the invited list for the same reason, and to `participants` because
-- the client has been asking for it all along.
