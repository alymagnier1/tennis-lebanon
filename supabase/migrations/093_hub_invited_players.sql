-- A host could not see who they had invited, or who had said no.
--
-- The hub renders its roster through the vs-hero, and `pickHubVsSides` builds
-- that from accepted participants only. The one component showing a per-player
-- status renders only when the hero does not, and the hero covers every live
-- status. So an invited player and an open slot nobody was ever asked to fill
-- looked identical, and the rational move for a host was to invite the same
-- person a second time.
--
-- Two things had to change, and the second is why this is a migration rather
-- than a screen fix.
--
-- **Invites are not participants.** `create_match_invite` writes to
-- `match_invitations`; no `match_participants` row exists until the invite is
-- accepted. Anything reading the roster to answer "who did I invite" is reading
-- the wrong table.
--
-- **A decline was indistinguishable from a withdrawal.**
-- `decline_match_invitation` set `revoked_at`, and so does
-- `revoke_pending_targeted_invites` when the host takes the offer back. One
-- column recorded two opposite events, so "did they say no?" was not merely
-- unexposed — it was not recorded. `declined_at` separates them. Invitations
-- revoked before this migration keep a null `declined_at` and are treated as
-- withdrawals, which is the safer reading: claiming somebody declined when they
-- may not have is worse than staying quiet.

alter table public.match_invitations
  add column if not exists declined_at timestamptz;

comment on column public.match_invitations.declined_at is
  'Set when the invited player declined. `revoked_at` alone cannot say whether the player refused or the host withdrew the offer.';

-- Declining still sets `revoked_at`, so every existing query that treats a
-- revoked invite as no longer pending keeps working untouched.
...
alter type public.match_hub_card
  add attribute invited_players jsonb;
...
begin
  ...
  -- Host-only, like pending_requests. Answers "who did I ask, and did they say
  -- no?", which nothing could: the hero renders accepted players and open
  -- slots, so an invited player and a slot nobody was asked to fill looked
  -- identical, and the rational move was to invite the same person twice.
  --
  -- Read from `match_invitations`, not `match_participants`: an invite does not
  -- create a participant row until it is accepted. `declined_at` separates the
  -- player saying no from the host withdrawing the offer — both still write
  -- `revoked_at`, so a host revoke is correctly absent from this list.
    ...
    select coalesce(
      jsonb_agg(
        jsonb_build_object(
          'user_id', p.id,
          'display_name', p.display_name,
          'status', case when mi.declined_at is not null then 'declined' else 'invited' end
        )
        order by mi.declined_at nulls first, p.display_name
      ),
      '[]'::jsonb
    )
