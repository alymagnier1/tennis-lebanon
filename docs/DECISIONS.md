...
- Status: accepted
- Context: A player who asked to join an approval-gated match had no way out. `leave_match` requires `status = 'accepted'` and otherwise raises "Not an active participant", so nothing covered a `requested` row: the request sat there until the host answered, and the asker could not change their mind. Finding 13 in `docs/COHORT_A_REHEARSAL_FINDINGS.md`.
- Decision: `092_withdraw_join_request.sql` adds `withdraw_join_request`, setting the caller's `requested` row to `left` and telling the host through a new `match_request_withdrawn` kind. Surfaced as a destructive link on the match hub, beside leave and withdraw.
- Alternatives considered: withdrawing to `declined` (rejected — that status records the host's answer, and `077` keys its decline notification on exactly the `requested → declined` transition, so reusing it would tell the asker their own withdrawal was a rejection from the host); extending `leave_match` to cover `requested` (rejected — its body is about roster and booking consequences a pending request does not have, and `match_not_leavable` would have to grow exceptions for a case that shares none of its rules); leaving the withdrawal silent (rejected — the host may be holding a slot for an answer, and `076` already sent them to a hub that would now have nothing pending on it).
- Consequences: `left` already reactivates on rejoin (`088`), so asking again after withdrawing needs no further change; the pgTAP asserts it. A fifth roster kind to keep in parity across `NOTIFICATION_KINDS`, three locale bundles and the Edge Function table, which the existing parity test guards. A player can now withdraw and re-request repeatedly — the 15-minute dedup bucket collapses the host's notifications, so this is noisy rather than abusive, and the invite rate limit does not cover the joining side. Worth watching in cohort A rather than pre-solving.
- Owner: Founder

## 2026-08-28 — Inviting a player is not sharing a link

- Status: accepted
- Context: Two findings on the invite screen. It showed anyone holding a `requested` row as **Invited**: `isAlreadyInMatch` collapsed `accepted`, `invited` and `requested` into one state and the resolver returned only two, so a player who had asked _you_ read as one you had asked, and the row most needing an answer looked like one already dealt with. Separately the invite button both sent a targeted invite and opened the OS share sheet, so a host sent the same match to one player twice — once by push, once by whatever they picked in the sheet. Findings 12 and 15.
